...
export const insertCustomerSchema = createInsertSchema(customer, {
    ...
    phone: (schema) => schema.regex(/^\d{3}-\d{3}-\d{4}$/, "Invalid phone number format").min(1, "Phone number is required"),
    address1: (schema) => schema.min(1, "Address is required"),
    city: (schema) => schema.min(1, "City is required"),
    state: (schema) => schema.length(2, "State must be 2 characters"),
    zip: (schema) => schema.regex(/^\d{5}(-\d{4})?$/, "Zip code must be 5 digits or 9 digits with a hyphen"),
